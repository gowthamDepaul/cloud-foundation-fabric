...
module "apigee" {
  ...
  organization = merge(var.apigee_config.organization, try(var.shared_vpc_config.use_for_apigee, false) ? {
    authorized_network = var.shared_vpc_config.id
    } : var.apigee_config.organization.disable_vpc_peering ? {} : {
    authorized_network = module.vpc[0].id
    },
